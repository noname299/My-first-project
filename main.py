...
REGISTERED_CHATS = {
    "@Because_You_Deserve_It"
}

with open("shia_shubuhat_10000_responses.txt", "r", encoding="utf-8") as f:
...
async def auto_send_content(context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id in REGISTERED_CHATS:
        try:
            ...
            await context.bot.send_message(chat_id=OWNER_ID, text=f"📢 تم الإرسال إلى: {chat_id}")
        except Exception as e:
            await context.bot.send_message(chat_id=OWNER_ID, text=f"⚠️ فشل الإرسال إلى {chat_id}: {str(e)}")

async def register_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    REGISTERED_CHATS.add(chat_id)
    try:
        await context.bot.send_message(chat_id=OWNER_ID, text=f"✅ تم إضافة البوت إلى: {update.effective_chat.title or chat_id}")
    except:
        pass
...
def main():
    ...
    app.job_queue.run_repeating(auto_send_content, interval=7200, first=10)
    app.run_polling()
